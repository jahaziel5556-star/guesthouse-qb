...
async function fetchJson(url, options = {}) {
  ...
  // Prefix all API calls with the Render backend URL if not absolute
  const apiBase = 'https://guesthouse-curl.onrender.com';
  let fullUrl = url;
  if (url.startsWith('/')) {
    fullUrl = apiBase + url;
  }
...
async function checkQuickBooksLogin() {
  try {
    ...
    try {
      const data = await fetchJson('/check-token', { method: 'GET' }); // Now uses Render backend
