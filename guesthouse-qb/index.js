// index.js - QuickBooks backend (post net amount; TaxExcluded; QBO computes tax to reach gross)
...
async function resolveTaxCodeByCode(tokens, codeOrName) {
  ...
  const ref = { value: tc.Id, _full: tc };
  taxCache.byNameOrId.set(key, ref);
  return ref;
}
...
async function resolveTaxCodeByAgency(tokens, agencyName) {
  ...
  for (const code of codes) {
    ...
    if (match) {
      const ref = { value: code.Id, _full: code };
...
async function resolveAnyVATCode(tokens) {
  ...
  taxCache.vatFallback = { value: hit.Id, _full: hit };
...
function extractCombinedRate(taxCodeFull) {
  ...
  return taxCodeFull.TaxRateList.TaxRateDetail.reduce((sum, d) => {
    const rate = parseFloat(d.RateValue || d.TaxRateRef?.RateValue || 0);
...
app.post("/payment-to-quickbooks", async (req, res) => {
  try {
    const {
      ...
      address,
      customerNumber,
      amount, // gross total paid by guest
      receiptNumber,
      date,
      room,
      checkin,
    ...
    // Compute net to guarantee gross = net + computed tax
    const netAmount =
    ...
    if (combinedRate > 0) {
      ...
      descParts.push(
        `VAT @ ${combinedRate.toFixed(2)}% on EC$${netAmount.toFixed(2)} = EC$${taxAmount.toFixed(2)} (Total EC$${grossAmount.toFixed(2)})`
      );
    }
    ...
    if (!customerId) {
      const found = await qboQuery(tokens, `select * from Customer where DisplayName='${name.replace(/'/g, "\\'")}'`);
      const cust = found.QueryResponse.Customer?.[0];
      if (cust) {
        customerId = cust.Id;
      } else {
    ...
    // Echo numbers for UI
    const finalTax = combinedRate > 0 ? +(grossAmount - netAmount).toFixed(2) : 0;
    res.json({
      success: true,
