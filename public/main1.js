 ...
 */
async function bulkResyncToQuickBooks(lastSentReceiptNumber = 17) {
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        await updateDoc(paymentRef, {
          ...
          unvoidedByName: currentEmployee?.displayName || empName || 'Unknown'
        });
...
document.getElementById("confirmPaymentBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (reservation && customer) {
      // Build complete QB data using helper function
      const payment = {
        id: paymentRef.id,
        receiptNumber: receipt,
        amount: amount,
...
async function openExtendReservationModal(reservation) {
  ...
  const validateExtensionData = () => {
    ...
    // Check departure is after current departure
    if (new Date(extensionNewDeparture) <= new Date(reservation.departureDate)) {
...
function showCustomerListModal() {
  ...
  // Set explicit background on the list container
  ...
  function renderList(filter = "") {
    ...
    filtered.forEach(c => {
      ...
      div.onmouseenter = () => { if (div !== selectedDiv) div.style.background = "#f5f5f5"; };
      div.onmouseleave = () => { if (div !== selectedDiv) div.style.background = "#fff"; };
      div.onclick = () => {
...
function showCustomerDetailsModal(customer) {
  ...
  // Fill fields (no internal ID shown)
  info.innerHTML = `
    <div style="margin-bottom:8px;"><strong>Name:</strong> ${customer.name || ""}</div>
    <div style="margin-bottom:8px;"><strong>Phone:</strong> ${customer.telephone || ""}</div>
    <div style="margin-bottom:8px;"><strong>Address:</strong> ${customer.address || ""}</div>
    ...
    modalContent.querySelector("#generateReceiptsBtn").onclick = async () => {
      ...
      for (let i = 0; i < pages; i++) {
        ...
        for (let p of slice) {
          ...
          const baseTotal = (parseFloat(p.rate) || 0) * nights;
          // Include balance adjustments
          const adjustments = reservation.balanceAdjustments || [];
          const totalAdjustment = adjustments.reduce((sum, adj) => {
            return sum + (adj.type === 'discount' ? -adj.amount : adj.amount);
 ...
 * Sends all unsent receipts to QuickBooks
 */
document.getElementById('bulkQBResyncBtn')?.addEventListener('click', async () => {
  const confirmed = confirm("This will send all receipts that haven't been synced to QuickBooks yet.\n\nDo you want to continue?");
  if (!confirmed) return;
